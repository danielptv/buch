...
function Dashboard {
    $port = '7070'
    $kubeviousPod = kubectl get pods --selector "app.kubernetes.io/component=kubevious-ui" --output jsonpath="{.items[0].metadata.name}" --namespace $namespace
    Write-Output ''
    Write-Output "Pod: $kubeviousPod"
    Write-Output "Aufruf in einem Webbrowser: http://localhost:$port"
    Write-Output ''
    kubectl port-forward pod/$kubeviousPod ${port}:80 --namespace $namespace
}
...
function Install {
    ...
    helm install $release kubevious/kubevious --namespace $namespace
}
